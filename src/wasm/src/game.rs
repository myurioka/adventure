...
impl StaticGame for Game{
    ...
    fn new(document: Document) -> Self{
        ...
        let _screen_width = _canvas.client_width().into();
        let _screen_height = _canvas.client_height().into();
    ...
    fn draw(&self){
        ...
        match _page {
            ...
            1 .. 20 => {
                ...
                if _page % 2 == 0 {_context.set_global_alpha(0.5);}; // at showing messages from AI
                ...
                let _= _context.set_fill_style_str(DEFAULT_COLOR);
                let _= _context.set_text_align("center");
                let _= _context.fill_text("【 LITTLE RED RIDING HOOD 】", 250.0, 30.0);
                ...
                let input_element = _input.dyn_into::<HtmlInputElement>().unwrap();
                let _= input_element.style().set_property("background-color", "#D9FFB3");
                let _= input_element.set_placeholder(TEXT_CHAPTER_TEXT_PLACEHOLDER[_chapter]);
                if _page % 2 == 0 {
                    let _= input_element.set_value("Touch or Click Screen");
                    let _= input_element.set_disabled(true);
                } else if _page == 0 {
                    let _= input_element.set_value("Enter Gemini API Key");
                } else {
                    let _= input_element.set_disabled(false);
                }
            },
            _ => {
            }
    ...
    fn show_message(&self){
        ...
        if self.page % 2 != 0 {
            ...
            let _html_textarea_message = _textarea.dyn_into::<HtmlTextAreaElement>().unwrap();
            let _= _html_textarea_message.style().set_property("visibility", "hidden");
